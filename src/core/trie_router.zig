...
pub const TrieNode = struct {
    ...
    pub fn search(self: *TrieNode, path: []const u8, method: HttpMethod, params: *RouteParams) ?Handler {
        return self.searchRecursive(path, method, params, 0);
    }
    
    fn searchRecursive(self: *TrieNode, path: []const u8, method: HttpMethod, params: *RouteParams, start_pos: usize) ?Handler {
        // Find next segment
        var pos = start_pos;
        ...
        if (pos >= path.len) {
            ...
            const method_index = @intFromEnum(method);
            return self.method_handlers[method_index];
        }
        ...
        if (self.children.get(segment)) |child| {
            if (child.searchRecursive(path, method, params, end_pos)) |handler| {
                return handler;
            }
        }
        ...
        if (self.wildcard_child) |child| {
            const method_index = @intFromEnum(method);
            return child.method_handlers[method_index];
        }
...
pub const TrieRouter = struct {
    ...
    pub fn findRoute(self: *TrieRouter, method: HttpMethod, path: []const u8) ?RouteMatch {
        ...
        if (trie.search(path, method, params)) |handler| {
            return RouteMatch{
                .handler = handler,
                .params = params,
            };
